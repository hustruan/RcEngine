...
void DeferredPath::DeferredLighting()
{
	...
	mDevice->BindFrameBuffer(mLightAccumulateFB);
	//mLightAccumulateFB->Clear(CF_Color | CF_Depth | CF_Stencil, ColorRGBA(0, 0, 0, 0), 1.0, 0);
	...
	for (Light* light : mSceneMan->GetSceneLights())
	{
		...
		else 
		{
			if (!stencilZFail)
			{
				mDepthStencilBufferLightView->ClearDepthStencil(1.0, 0);
				...
				// D3D11 doesn't support copy to DepthStencil texture
